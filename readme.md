...
> R: Se prefiere revert porque no es destructivo. Permite corregir errores manteniendo la transparencia y evita que el resto del equipo tenga que corregir errores manualmente
```
...
> R:Importa principalmente en sistemas embebidos o programación de bajo nivel donde el rendimiento es crítico. La operación de bits (&) suele ser más veloz y requiere menos ciclos de reloj del procesador que una división (%). Sin embargo, en programación general se prefiere % porque es mucho más fácil de leer para otros programadores y los compiladores modernos suelen optimizarlo automáticamente para que rinda igual de bien que la operación de bits.
...
> R:Le explicaría que es mejor commitear cada vez que termine una tarea pequeña o una funcionalidad lógica que funcione. "Commitear seguido y pushear cuando estés seguro"

**P4** — ¿Cuál es la diferencia entre `git fetch` y `git pull`? ¿Cuándo preferirías usar uno sobre el otro?

> R: git fetch descarga la información del servidor en las ramas de seguimiento remoto sin modificar tus archivos locales, permitiéndote revisar los cambios antes de integrarlos; en cambio, git pull es la unión de un fetch y un merge, por lo que descarga y combina automáticamente los cambios remotos en tu rama actual. Se prefiere usar fetch cuando se necesita comparar versiones y evitar conflictos inesperados, mientras que pull es ideal para actualizarse rápidamente cuando se tiene la seguridad de que la rama está limpia y lista para la integración.

**P5** — ¿Qué información debería tener la descripción de un Pull Request para que sea útil para quien lo revisa?

> R:Una descripción útil de un Pull Request debe explicar claramente el "qué" y el "porqué" de los cambios, incluyendo un resumen de las funcionalidades añadas o errores corregidos. Es fundamental mencionar el contexto o la motivación técnica detrás del cambio y, si es posible, detallar los pasos para que quien revisa pueda testear el código y verificar que funciona correctamente. Finalmente, es una buena práctica listar cualquier dependencia nueva o advertencia sobre efectos secundarios que el cambio pueda tener en otras partes del sistema para facilitar una revisión segura y eficiente.
